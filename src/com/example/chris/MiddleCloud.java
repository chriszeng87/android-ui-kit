...
/*中型飞机的类*/
public class MiddleCloud  extends GameObject{
	private Bitmap middleCloud;
//	private int blood; 		// 对象的当前血量
//	private int bloodVolume;  // 对象的血量
	MiddleCloud(Resources resources) {
		super(resources);
...
//		blood = bloodVolume;
	...
	public void initBitmap() {
		...
		middleCloud = BitmapFactory.decodeResource(resources, R.drawable.cloud2);
		object_width = middleCloud.getWidth();		//获得每一帧位图的宽
		object_height = middleCloud.getHeight();///4;		//获得每一帧位图的高
	}
	//绘图函数
	@Override
	public void drawSelf(Canvas canvas) {
		...
		if(isAlive){
			if(!isExplosion){
				...
				canvas.drawBitmap(middleCloud, object_x, object_y,paint);
				canvas.restore();
				logic();
			}
//			else{
