...
public class EnemyPlane extends GameObject{
	...
	EnemyPlane(Resources resources) {
		super(resources);
	...
	public void initBitmap() {
		// TODO Auto-generated method stub
		 enemyPlane = BitmapFactory.decodeResource(resources, R.drawable.enemy);
		 object_width = enemyPlane.getWidth();		//获得每一帧位图的宽
		 object_height = enemyPlane.getHeight();///3;		//获得每一帧位图的高
	}
	//绘图函数
	@Override
	public void drawSelf(Canvas canvas) {
		...
		if(isAlive){
			if(!isExplosion){
				...
				canvas.drawBitmap(enemyPlane, object_x, object_y  ,paint);
				canvas.restore();
				logic();
			}
			else{
				...
				canvas.save();
				canvas.clipRect(object_x,object_y,object_x + object_width,object_y + object_height);
				canvas.drawBitmap(enemyPlane, object_x, object_y - y,paint);
				canvas.restore();
				currentFrame++;
				if(currentFrame >= 3){
					currentFrame = 0;
